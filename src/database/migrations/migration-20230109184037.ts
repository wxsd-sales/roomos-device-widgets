...
export class Migration20230109184037 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `user` (`uuid` text not null, `created_at` datetime not null default current_timestamp, `updated_at` datetime not null default current_timestamp, `webex_uuid` text null, `email` text not null, primary key (`uuid`));');
