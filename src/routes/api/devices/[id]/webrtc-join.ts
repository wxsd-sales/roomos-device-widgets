...
import { IsIn, IsNotEmpty, IsOptional, IsUrl, Matches, MaxLength, MinLength, validateSync } from 'class-validator';
...
export const POST: RequestHandler = async (requestEvent: RequestEvent) => {
  ...
  class RequestBodyDTO {
    ...
    @Expose()
    @IsOptional()
    @IsIn(['MSTeams', 'GoogleMeet'])
    public readonly type?: 'MSTeams' | 'GoogleMeet';
  ...
  return webexHttp(header.authorization, 'xapi')
    .post('command/WebRTC.Join', undefined, {
      deviceId: params.id,
