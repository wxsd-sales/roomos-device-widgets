<script lang="ts">
  ...
  export let insertEvent: (...args) => Promise<Response> = () => Promise.reject(undefined);
  ...
  const handleClick = () =>
    Promise.resolve((isLoading = true))
      .then(() => insertEvent(new Date(start), new Date(end), attendees, summary, description))
      .then(() => onSuccess())
...
<form class="columns is-multiline">
  ...
  <div class="column is-full">
    ...
    <div class="control has-icons-left">
      <input class="input is-medium is-rounded" bind:value={summary} required />
