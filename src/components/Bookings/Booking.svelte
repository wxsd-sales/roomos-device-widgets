<script lang="ts">
  ...
  export let organizer: { name: string; email?: string };
  export let protocol: 'SIP' | 'H323' | 'ISDN' | 'IP' | 'Spark' | 'WebRTC' = 'SIP';
...
<div class="columns is-mobile">
  <div class="column is-7 has-text-weight-medium">
    ...
    {/if}
    <p class="is-size-6 has-text-grey-light">Reserved by {organizer.name || organizer.email || 'unknown'}</p>
  </div>
